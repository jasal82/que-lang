//! std.yaml module — YAML parsing and serialization.
...
impl Interpreter {
    pub(crate) fn call_yaml(&mut self, func: &str, args: &[Value]) -> IResult {
        match func {
            "parse" => {
                let s = arg_str(args, 0, "yaml.parse")?;
                match crate::config::parse_yaml(s) {
            ...
            "stringify" => {
                let value = args.first().ok_or_else(|| {
                    Signal::Error(QueError::new(ErrorKind::ArityMismatch, "yaml.stringify requires 1 argument"))
                })?;
                match crate::config::to_yaml(value) {
            ...
            "edit" => {
                if args.len() < 2 {
                    return Err(Signal::Error(QueError::new(
                        ...
                        "yaml.edit requires 2 arguments: path, fn(doc) -> doc",
                    )));
                }
                let p = path_str(&args[0], "yaml.edit")?;
                let func_val = args[1].clone();
                let content = std::fs::read_to_string(&p).map_err(|e| {
                    Signal::Error(QueError::new(ErrorKind::IoError, e.to_string()))
                })?;
                let doc = crate::config::parse_yaml(&content).map_err(|e| {
            ...
            _ => Err(Signal::Error(QueError::new(
                ErrorKind::Runtime,
                format!("unknown function 'yaml.{}'", func),
            ))),
        }
    }
}
