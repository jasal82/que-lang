...
pub struct Interpreter {
    ...
    /// Types this module took in through a `pub import` and therefore passes
    /// on. A type's fields and methods live in the tables above, not in the
    /// value that names it, so re-exporting the name alone would hand the
    /// importer a type it cannot construct or call.
    pub(crate) reexported_type_names: std::collections::BTreeSet<String>,
...
impl Interpreter {
    pub fn new() -> Self {
        ...
        let mut interp = Self {
            ...
            reexported_type_names: std::collections::BTreeSet::new(),
            call_stack: Vec::new(),
            strict: false,
            log_level: 0,
...
"#;
    ...
    fn exec_import(&mut self, decl: &ImportDecl) -> IResult {
        ...
        // A `pub import` passes these on in turn, so note the names: the
        // module's own exports are collected from its declarations, which
        // say nothing about a type that merely travelled through.
        for (name, fields) in loader.pending_struct_defs.drain() {
            if decl.is_pub {
                self.reexported_type_names.insert(name.clone());
            }
        ...
        for (key, methods) in loader.pending_trait_impls.drain() {
            if decl.is_pub {
                self.reexported_type_names.insert(key.0.clone());
            }
            self.trait_impls.entry(key).or_default().extend(methods);
        }
        self.module_loader = Some(loader);
