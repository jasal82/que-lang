...
pub struct Interpreter {
    ...
    /// Every type this module can reach: the ones it declares plus the ones
    /// that arrived through an import. A type's fields and methods live in the
    /// tables above rather than in the value that names it, so a module has to
    /// hand this set on to its importers — otherwise a function it exports
    /// would lose the ability to construct or call the types it was written
    /// against. Builtin types are deliberately absent: every interpreter
    /// registers those for itself, and copying them around would pile up
    /// duplicate method entries on every import.
    pub(crate) known_type_names: std::collections::BTreeSet<String>,
...
impl Interpreter {
    pub fn new() -> Self {
        ...
        let mut interp = Self {
            ...
            known_type_names: std::collections::BTreeSet::new(),
            call_stack: Vec::new(),
            strict: false,
            log_level: 0,
...
"#;
        if let Ok(mut m) = Self::parse_single_method(td_new_src) {
            // The body names TempDir, so it needs an environment where that
            // name is bound -- the one this method is parsed with is empty.
            m.closure_env = self.env.clone();
    ...
    fn exec_item(&mut self, item: &Item) -> IResult {
        match item {
            ...
            Item::EnumDecl(decl) => {
                ...
                self.known_type_names.insert(decl.name.clone());
            ...
            Item::ImplDecl(decl) => {
                ...
                self.impl_methods
                    ...
                    .or_default()
                    .extend(methods);
                self.known_type_names.insert(decl.type_name.clone());
                Ok(Value::Null)
            }
            Item::TraitDecl(decl) => {
    ...
    fn exec_import(&mut self, decl: &ImportDecl) -> IResult {
        ...
        // Merge type metadata from the imported module into this interpreter,
        // and record the names so we pass them on in turn. This happens for
        // every import, not just a `pub` one: a plain import still leaves us
        // holding functions and values that were written against those types,
        // and whoever imports *us* will end up running them.
        ...
        for (key, methods) in loader.pending_trait_impls.drain() {
            self.known_type_names.insert(key.0.clone());
            self.trait_impls.entry(key).or_default().extend(methods);
        }
        self.module_loader = Some(loader);
